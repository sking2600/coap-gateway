...
import (
	...
	"github.com/sking2600/coap-gateway/internal/registry"
...
func main() {
	...
	//run server
	//dbURI only commented out so I don't have to fiddle with env vars in local development
	//dbURI := fmt.Sprintf("%s:%s%s%s%s", dbUsername, dbPassword, dbAddress, dbName, dbParameters)
	dbURI := "ocf_test:ocfftw!!@tcp(db4free.net:3306)/ocf_test?parseTime=true"
	//fmt.Println(dbURI)
